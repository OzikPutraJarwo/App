function hitungDMRT() {
  ...
  const elementDMRTP = document.querySelector('.snk-p');
  elementDMRTP.innerHTML = `<th>P</th>`;
  const elementDMRTT = document.querySelector('.snk-table');
  elementDMRTT.innerHTML = `<th>Tabel SNK</th>`;
  const elementDMRTC = document.querySelector('.snk-calc');
  elementDMRTC.innerHTML = `<th>SNK Hitung</th>`;
...
function processData() {
  ...
  function checkGreen() {
    ...
    matrixTds.forEach(matrixTd => {
      ...
      if (!dataP) return;
      const dmrtTd = document.querySelector(`tr.snk-calc td[data-p="${dataP}"]`);
      if (!dmrtTd) return;
